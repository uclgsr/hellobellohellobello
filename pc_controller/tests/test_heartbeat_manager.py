...
class TestHeartbeatManager:
    ...
    async def test_check_heartbeats_marks_devices_offline(self):
        ...
        # Set heartbeat time to past to simulate timeout
...
class TestHeartbeatIntegration:
    ...
    async def test_full_heartbeat_cycle(self):
        ...
        manager = HeartbeatManager(heartbeat_interval_s=0.05)  # Very fast for testing
        ...
        # Register device with callbacks
        manager.register_device("test_device")
        manager.set_device_offline_callback("test_device", offline_callback)
        ...
        # Device should be marked offline due to no heartbeats
        assert "test_device" in manager.get_unhealthy_devices()
        assert "offline:test_device" in events
        
        # Send a heartbeat to restore connection
        manager.record_heartbeat("test_device")
        
        # Device should be back online
        assert "test_device" in manager.get_healthy_devices()
        assert "online:test_device" in events
