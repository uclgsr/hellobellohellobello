...
class LSLOutletManager:
    ...
    def create_gsr_outlet(self, device_id: str, sample_rate: float = 50.0) -> bool:
        ...
        try:
            ...
            for i, (name, unit) in enumerate(zip(config.channel_names, config.channel_units)):
    ...
    def create_thermal_outlet(self, device_id: str, width: int = 256, height: int = 192, sample_rate: float = 10.0) -> bool:
    ...
    def stream_gsr_sample(self, device_id: str, gsr_value: float, ppg_value: float, timestamp: Optional[float] = None) -> bool:
    ...
    def stream_thermal_frame(self, device_id: str, frame: np.ndarray, timestamp: Optional[float] = None) -> bool:
    ...
    def remove_outlet(self, device_id: str, sensor_type: str) -> bool:
        ...
        if outlet_name in self._outlets:
            try:
                ...
                logger.error(f"Error removing {sensor_type} outlet for {device_id}: {e}")
                return False
...
_lsl_manager: Optional[LSLOutletManager] = None
