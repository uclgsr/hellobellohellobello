...
class Shimmer3BLEAndroid(
    ...
    override fun connect(bluetoothAddress: String, deviceName: String) {
    ...
    private fun scanAndConnect(targetAddress: String, deviceName: String) {
        if (mIsScanning) {
    ...
    private fun processIncomingData(data: ByteArray) {
        ...
        mDataProcessingJob = mDataProcessingScope?.launch {
    ...
    private fun parseDataPacket(packet: ByteArray) {
        try {
            ...
            val timestamp = ((packet[offset].toInt() and 0xFF) or
                           ((packet[offset + 1].toInt() and 0xFF) shl 8) or
                           ((packet[offset + 2].toInt() and 0xFF) shl 16)).toLong()
            offset += TIMESTAMP_BYTES
            
            // Extract GSR data (2 bytes, little-endian, 12-bit ADC)
            val gsrRaw = ((packet[offset].toInt() and 0xFF) or
                         ((packet[offset + 1].toInt() and 0xFF) shl 8)) and 0x0FFF  // Mask to 12-bit
            offset += GSR_BYTES
            
            // Extract PPG data (2 bytes, little-endian)
            val ppgRaw = ((packet[offset].toInt() and 0xFF) or
                         ((packet[offset + 1].toInt() and 0xFF) shl 8)) and 0x0FFF
            offset += PPG_BYTES
            ...
            objectCluster.addData(
                ...
                Configuration.Shimmer3.CHANNEL_TYPE.CAL.toString(),
                "mS",
                timestampCal
            )
    ...
    private fun startSimulationMode() {
        ...
        mSimulationJob = mDataProcessingScope?.launch {
