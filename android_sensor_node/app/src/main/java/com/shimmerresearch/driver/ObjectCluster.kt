...
class ObjectCluster {
     ...
     */
    fun addData(sensorName: String, formatType: String, units: String, data: Double) {
     ...
     */
    fun getCollectionOfFormatClusters(sensorName: String): Collection<FormatCluster>? {
        return mDataMap[sensorName]?.values
    }
    
    /**
     * Get specific format cluster
     */
    fun getFormatCluster(sensorName: String, formatType: String): FormatCluster? {
        return mDataMap[sensorName]?.get(formatType)
    }
     ...
     */
    companion object {
        fun returnFormatCluster(formatClusters: Collection<FormatCluster>, formatType: String): FormatCluster? {
            return formatClusters.find { it.mChannelType == formatType }
        }
    }
