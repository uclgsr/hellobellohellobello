...
class RealTopdonIntegration(private val context: Context) {
    ...
    enum class ThermalPalette {
        IRON, RAINBOW, GRAYSCALE, HOT, COOL
    }
    ...
    private fun scanForTC001Hardware(): UsbDevice? {
        ...
        for (device in deviceList.values) {
            Log.d(TAG, "Found USB device: VID=${String.format("0x%04X", device.vendorId)}, " +
     ...
     */
    suspend fun startStreaming(callback: (ThermalFrame) -> Unit): Boolean = withContext(Dispatchers.IO) {
     ...
     */
    private fun captureFromHardware(timestamp: Long, device: UsbDevice): ThermalFrame {
     ...
     */
    private fun generateThermalBitmap(temperatureData: FloatArray, minTemp: Float, maxTemp: Float): Bitmap {
