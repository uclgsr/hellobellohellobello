 ...
 */
class ThermalControlsView @JvmOverloads constructor(
    context: Context,
    ...
    init {
        ...
        deviceStatusText = TextView(context).apply {
        ...
        deviceStatusIndicator = View(context).apply {
            layoutParams = LayoutParams(50, 20).apply {
                bottomMargin = 16
            }
        ...
        paletteSpinner = Spinner(context).apply {
        ...
        emissivityValue = TextView(context).apply {
            text = "0.95"
            textSize = 12f
        }
        addView(emissivityValue)
        
        emissivitySlider = SeekBar(context).apply {
            max = 90
            progress = 85
        }
        ...
        minTempInput = EditText(context).apply {
            setText("-20")
        ...
        maxTempInput = EditText(context).apply {
            setText("150")
            inputType = android.text.InputType.TYPE_CLASS_NUMBER or android.text.InputType.TYPE_NUMBER_FLAG_SIGNED
        }
        addView(maxTempInput)
        
        tempRangeDisplay = TextView(context).apply {
            text = "-20.0°C - 150.0°C"
            textSize = 12f
            setPadding(0, 8, 0, 0)
        }
        ...
        // Auto Gain Control
        autoGainToggle = Switch(context).apply {
        ...
        temperatureCompensationToggle = Switch(context).apply {
        ...
        currentTempDisplay = TextView(context).apply {
            text = "Current: N/A"
            textSize = 16f
            setPadding(0, 16, 0, 0)
    ...
    private fun setupControls() {
        ...
        // Emissivity slider
        emissivitySlider.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
    ...
    fun updateDeviceStatus(status: String, isConnected: Boolean) {
