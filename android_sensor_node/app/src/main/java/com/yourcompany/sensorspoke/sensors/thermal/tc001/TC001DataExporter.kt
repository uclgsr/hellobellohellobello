 ...
 */
class TC001DataExporter(private val context: Context) {
    ...
    ): TC001ExportResult = withContext(Dispatchers.IO) {
     ...
     */
    private suspend fun exportTemperatureDataCSV(sessionDir: File, exportDir: File): File = withContext(Dispatchers.IO) {
        ...
        writer.use {
            ...
            repeat(1000) { index ->
                ...
                it.write("$timestamp,${String.format("%.2f", centerTemp)},${String.format("%.2f", minTemp)},${String.format("%.2f", maxTemp)},${String.format("%.2f", avgTemp)},$emissivity\n")
            }
        }
     ...
     */
    private suspend fun exportTemperatureDataJSON(sessionDir: File, exportDir: File): File = withContext(Dispatchers.IO) {
     ...
     */
    private suspend fun exportThermalImages(sessionDir: File, exportDir: File): List<File> = withContext(Dispatchers.IO) {
     ...
     */
    private suspend fun exportSessionMetadata(sessionId: String, sessionDir: File, exportDir: File): File = withContext(Dispatchers.IO) {
     ...
     */
    private suspend fun exportBinaryThermalData(sessionDir: File, exportDir: File): File = withContext(Dispatchers.IO) {
     ...
     */
    private suspend fun exportResearchDataPackage(sessionId: String, sessionDir: File, exportDir: File): File = withContext(Dispatchers.IO) {
        val packageFile = File(exportDir, "research_package.json")
        
        val researchPackage = JSONObject().apply {
            put("package_format", "TC001_RESEARCH_V1.0")
            put("session_id", sessionId)
            put("export_timestamp", System.currentTimeMillis())
            
            // Device specifications for research
            put("device_specifications", JSONObject().apply {
            ...
            put("calibration_data", JSONObject().apply {
                put("emissivity", 0.95)
     ...
     */
    private suspend fun createCompressedArchive(exportDir: File, sessionId: String): File = withContext(Dispatchers.IO) {
     ...
     */
    fun estimateExportSize(sessionDir: File, format: TC001ExportFormat): TC001ExportSizeEstimate {
        val thermalDataSizeMB = when (format) {
