 ...
 */
class TC001DeviceManager(private val context: Context) {
     ...
     */
    private suspend fun performDeviceScan(): List<TC001Device> = withContext(Dispatchers.IO) {
     ...
     */
    suspend fun connectToDevice(device: TC001Device): Boolean = withContext(Dispatchers.IO) {
        ...
        connectionJob = launch {
            try {
                ...
                val connectionResult = withTimeout(CONNECTION_TIMEOUT_MS) {
     ...
     */
    suspend fun disconnectFromDevice() = withContext(Dispatchers.IO) {
     ...
     */
    suspend fun attemptReconnection() = withContext(Dispatchers.IO) {
    ...
    private fun determineModelName(usbDevice: UsbDevice): String {
        return usbDevice.productName ?: "TC001"
    }
    ...
    private fun generateSerialNumber(): String {
        return "TC001-SIM-${System.currentTimeMillis().toString().takeLast(6)}"
    }
