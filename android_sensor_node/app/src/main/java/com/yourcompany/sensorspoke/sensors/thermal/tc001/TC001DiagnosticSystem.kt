 ...
 */
class TC001DiagnosticSystem(private val context: Context) {
     ...
     */
    suspend fun runComprehensiveDiagnostics(): TC001DiagnosticResults = withContext(Dispatchers.IO) {
        try {
            ...
            val results = TC001DiagnosticResults(
    ...
    private fun testHardwareConnectivity(): TC001DiagnosticTest {
        return try {
            ...
            val tc001Devices = deviceList.values.filter { device ->
                isTC001Device(device)
            }
            
            val testResult = if (tc001Devices.isNotEmpty()) {
            ...
            TC001DiagnosticTest(
                ...
                result = testResult,
                message = if (testResult == TC001DiagnosticTestResult.PASS) 
                ...
                recommendation = if (testResult == TC001DiagnosticTestResult.FAIL) 
    ...
    private fun testUSBCommunication(): TC001DiagnosticTest {
        return try {
            ...
            val testResult = if (speedMbps >= MIN_USB_SPEED_MBPS) {
                TC001DiagnosticTestResult.PASS
            } else {
            ...
            TC001DiagnosticTest(
                ...
                recommendation = if (testResult == TC001DiagnosticTestResult.PASS) 
    ...
    private fun testSystemPerformance(): TC001DiagnosticTest {
        return try {
            ...
            TC001DiagnosticTest(
                ...
                recommendation = when (testResult) {
    ...
    private fun testThermalProcessing(): TC001DiagnosticTest {
        return try {
            ...
            val thermalData = FloatArray(256 * 192) { index ->
            ...
            TC001DiagnosticTest(
                ...
                details = "Min: ${String.format("%.1f", minTemp)}°C, Max: ${String.format("%.1f", maxTemp)}°C, Avg: ${String.format("%.1f", avgTemp)}°C",
                recommendation = when (testResult) {
     ...
     */
    suspend fun generateDiagnosticReport(results: TC001DiagnosticResults): File = withContext(Dispatchers.IO) {
