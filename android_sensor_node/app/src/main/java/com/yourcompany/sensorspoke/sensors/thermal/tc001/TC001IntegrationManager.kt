 ...
 */
class TC001IntegrationManager(private val context: Context) {
     ...
     */
    suspend fun startSystem(): Boolean = withContext(Dispatchers.IO) {
        if (!isInitialized) {
     ...
     */
    suspend fun stopSystem() = withContext(Dispatchers.IO) {
        try {
