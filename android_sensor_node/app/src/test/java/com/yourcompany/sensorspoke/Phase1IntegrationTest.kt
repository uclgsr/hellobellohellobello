...
class Phase1IntegrationTest {
    ...
    fun `test RecordingController session lifecycle`() = runBlocking {
        ...
        assertTrue(controller.state.value == RecordingController.State.RECORDING, 
                   "Controller should be in RECORDING state")
        assertTrue(controller.currentSessionId.value == "test_session_123", 
        ...
        // Verify final state
        assertTrue(controller.state.value == RecordingController.State.IDLE, 
    ...
    fun `test NetworkClient NSD registration format`() {
        ...
        try {
            ...
            assertTrue(e.message?.contains("NsdManager") == true || 
    ...
    fun `test StubSensorRecorder functionality`() = runBlocking {
        ...
        assertTrue(dataFile.readText().contains("timestamp,value"), 
