...
class ThermalIntegrationTest {
    ...
    fun testCSVFormatCompliance() = runBlocking {
        ...
        val expectedColumns = listOf(
            "timestamp_ns", "frame_id", "center_temp_c",
            "min_temp_c", "max_temp_c", "avg_temp_c", "image_path"
        )
        ...
        if (lines.size > 1) {
            ...
            assertTrue("Center temperature should be reasonable",
                centerTemp > -50.0f && centerTemp < 200.0f)
        }
    }

    @Test
    fun testThermalImageGeneration() = runBlocking {
        recorder.start(testSessionDir)
        Thread.sleep(500)  // Let multiple frames generate
        recorder.stop()

        // Check for generated thermal images
        val imageFiles = testSessionDir.listFiles { _, name ->
        ...
        assertTrue("Image should follow naming convention",
            firstImage!!.name.matches(Regex("thermal_frame_\\d{6}\\.png")))
    }

    @Test
    fun testMetadataContent() = runBlocking {
        recorder.start(testSessionDir)
        recorder.stop()
        ...
        val requiredFields = listOf(
            "sensor_type", "device_model", "resolution_width",
            "resolution_height", "temperature_range_min", "temperature_range_max"
        )

        for (field in requiredFields) {
            assertTrue("Metadata should contain $field",
                metadataContent.contains("\"$field\""))
        }

        // Verify Topdon TC001 is mentioned
        assertTrue("Should specify Topdon TC001",
            metadataContent.contains("Topdon TC001"))

        // Verify resolution specifications
        assertTrue("Should specify 256x192 resolution",
            metadataContent.contains("256") && metadataContent.contains("192"))
    }

    @Test
    fun testTemperatureRangeValidation() = runBlocking {
        recorder.start(testSessionDir)
        Thread.sleep(300)
        recorder.stop()
        ...
        if (csvFile.exists()) {
            ...
            for (i in 1 until lines.size) { // Skip header
                ...
                if (data.size >= 6) {
                    ...
                    assertTrue("Temperatures should be reasonable",
    ...
    @Test
    fun testSimulationMode() = runBlocking {
        ...
        if (temperatures.size > 5) {
            ...
            val variance = temperatures.map { temp ->
                val mean = temperatures.average()
                (temp - mean) * (temp - mean)
            }.average()

            assertTrue("Simulation should generate varied temperatures",
                variance > 0.1) // Some minimum variance
        }
    }
