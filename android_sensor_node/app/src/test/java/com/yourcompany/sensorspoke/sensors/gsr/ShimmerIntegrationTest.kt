...
class ShimmerIntegrationTest {
    ...
    @Test
    fun testBasicRecordingLifecycle() = runBlocking {
        // Start recording
    ...
    @Test
    fun testCSVFormatCompliance() = runBlocking {
        recorder.start(testSessionDir)
        ...
        if (lines.size > 1) {
            ...
            assertEquals("Data row should have correct number of columns",
                expectedColumns.size, dataRow.size)

            // Validate timestamp format (should be nanoseconds)
            val timestamp = dataRow[0].toLong()
            assertTrue("Timestamp should be reasonable nanosecond value",
                timestamp > 1_000_000_000_000_000L) // > year 2001 in ns
            ...
            if (ppgValue != null) {
                assertTrue("PPG should be reasonable 12-bit range",
                    ppgValue >= 0 && ppgValue <= 4095)
            }

            // Validate raw ADC value - CRITICAL 12-bit requirement
            val rawAdc = dataRow[3].toInt()
            assertTrue("Raw ADC should be in 12-bit range (0-4095)",
                rawAdc >= 0 && rawAdc <= 4095)
        }
    }

    @Test
    fun testCritical12BitADCRequirement() = runBlocking {
        ...
        for (i in 1 until lines.size) { // Skip header
            ...
            if (data.size >= 4) {
                ...
                assertTrue("Raw ADC MUST use 12-bit resolution (0-4095)",
                    rawAdc >= 0 && rawAdc <= 4095)
    ...
    fun testGSRConversionAccuracy() {
        ...
        assertEquals("16-bit values should be clamped to 12-bit max",
            maxValidResult, clampedResult, 0.001)
    }

    @Test
    fun testSamplingRateCompliance() = runBlocking {
        recorder.start(testSessionDir)
        ...
        if (lines.size > 10) { // Need enough samples
            ...
            if (timestamps.size >= 2) {
                ...
                assertTrue("Sampling rate should be close to 128 Hz, got $actualRate",
    ...
    @Test
    fun testSimulationModeRealism() = runBlocking {
        ...
        if (lines.size > 20) { // Need sufficient data
            ...
            if (gsrValues.isNotEmpty()) {
                ...
                assertTrue("Mean GSR should be realistic (5-50 µS)",
                    mean >= 5.0 && mean <= 50.0)
                ...
                assertTrue("All GSR values should be positive",
    ...
    @Test
    fun testPPGSimulation() = runBlocking {
        recorder.start(testSessionDir)
        Thread.sleep(400)
        recorder.stop()
        ...
        for (i in 1 until lines.size) {
            ...
            if (data.size >= 3) {
                ...
                if (ppgRaw != null && ppgRaw != 0) {
                    ...
                    assertTrue("PPG should be in 12-bit range",
                        ppgRaw >= 0 && ppgRaw <= 4095)

                    // Should be around mid-range for typical PPG
                    assertTrue("PPG should be reasonable physiological range",
    ...
    @Test
    fun testBLEConnectionSimulation() = runBlocking {
    ...
    fun testErrorHandling() = runBlocking {
        ...
        try {
            ...
            assertTrue("Should be a reasonable exception type",
    ...
    @Test
    fun testTimestampMonotonicity() = runBlocking {
        recorder.start(testSessionDir)
        Thread.sleep(500)
        recorder.stop()
        ...
        if (lines.size > 3) { // Need multiple data points
            ...
            for (i in 1 until timestamps.size) {
                assertTrue("Timestamps should be monotonically increasing",
